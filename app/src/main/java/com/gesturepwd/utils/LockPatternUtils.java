...
public class LockPatternUtils {
	...
	//TODO 转换成字符密码
	public static String convertToString(List<LockPatternView.Cell> list){
		String pwd ="";
		for (int i = 0;i<list.size();i++){
			if (list.get(i).getRow()==0){
				pwd = pwd+String.valueOf(list.get(i).getColumn()+1);
			}else if (list.get(i).getRow()==1){
				pwd = pwd+String.valueOf(list.get(i).getColumn()+4);
			}else if (list.get(i).getRow()==2){
				pwd = pwd+String.valueOf(list.get(i).getColumn()+7);
			}
		}
		return pwd.trim();
	}
