...
import static com.agentweb.FileUpLoadChooserImpl.REQUEST_CODE;
...
public final class ActionActivity extends Activity {
    ...
    private void openRealFileChooser() {
        try {
            ...
            this.startActivityForResult(Intent.createChooser(intent, "File Chooser"), REQUEST_CODE);
        } catch (Throwable throwable) {
            ToastUtil.toastShow(this, "找不到文件选择器");
...
//            if (mFileDataListener != null)
            mFileDataListener.onFileDataResult(requestCode, Activity.RESULT_OK, mUri != null ? new Intent().putExtra(KEY_URI, mUri) : data);
            mFileDataListener = null;
            finish();
        }
    }
